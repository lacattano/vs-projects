...
from cli.config import config, ReportFormat, JIRA_PROJECT_KEY
...
class JiraReportGenerator:
    ...
    def create_test_case(self, analyzed_case: AnalyzedTestCase, 
        ...
        # Generate unique key
        safe_title = re.sub(r'[^\w\s-]', '', analyzed_case.title.lower().replace(' ', '-'))[:50]
    ...
    def add_execution_result(self, test_case: JiraTestCase, 
...
'''
            ...
            if tc.screenshots:
                ...
                for screenshot in tc.screenshots:
                    ...
                    html_parts.append(
                        f'<img class="screenshot" src="{screenshot}" alt="{screenshot_name}">'
                    )
    ...
    def generate_jira_xml(self, output_path: str) -> str:
        ...
        for tc in self.test_cases:
            screenshots_xml = "\n".join(
                f'<screenshot>{os.path.basename(s)}</screenshot>' 
                for s in tc.screenshots
            )
    ...
    def _save_markdown(self, output_path: str) -> str:
        ...
        for tc in self.test_cases:
            lines.extend([
                ...
                "#### Expected Results",
                "",
                tc.expected_results.replace("<h3>Specific Expectations</h3>", "").replace("<ul><li>", "- ").replace("</li></ul>", ""),
                "",
                "---",
                ""
            ])
