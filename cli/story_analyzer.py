    ...
    def suggest_data(cls, text: str) -> Dict[str, Any]:
        ...
        if 'form' in categories or 'submit' in text_lower:
            suggested_data['form_data'] = {
                'name': "Test User",
                'email': cls._generate_test_email()
            }
...
class DependencyAnalyzer:
    ...
    COMMON_PRECONDITIONS = [
        'login', 'authentication', 'be logged in', 'be authenticated',
        'create account', 'register', 'sign up', 'navigate to login'
    ]
    
    COMMON_POSTCONDITIONS = [
        'logout', 'clear form', 'reset', 'navigate to home', 'go home'
    ]
...
class UserStoryAnalyzer:
    ...
    def analyze(self, parsed_input: ParsedInput) -> AnalysisResult:
        ...
        summary = {
            ...
            "detected_actions": list(set([a for case in analyzed_cases for a in case.identified_actions])),
            "detected_expectations": list(set([e for case in analyzed_cases for e in case.identified_expectations])),
            "requires_auth": any('login' in case.description.lower() or 'authentication' in case.description.lower() for case in analyzed_cases),
            "analysis_timestamp": datetime.now().isoformat()
        }
...
class InputEnricher:
    ...
    def enrich(self, text: str, explicit_format: Optional[str] = None) -> AnalysisResult:
        """
...
def analyze_user_story(story: str, analysis_mode: Optional[AnalysisMode] = None) -> AnalysisResult:
